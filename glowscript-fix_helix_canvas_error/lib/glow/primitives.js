...
//	            $(attrs.jcheckbox).attr('disabled', attrs.disabled)
	...
	function winput(args) { // a winput (widget inut) lets user input a string or a number
	    ...
	    if (!(this instanceof winput)) return new winput(args)  // so winput() is like new winput()
	    args = args || {}
	    var cvs = canvas.get_selected()
	    var attrs = {pos:cvs.caption_anchor, text:'', width:100, height:20, type:'numeric', number:null, disabled:false}
	    if (args.bind !== undefined) {
	    	attrs.bind = args.bind
	    	delete args.bind
	    } else throw new Error("A winput must have a bind attribute.")
	    for (var a in attrs) {
	    	if (args[a] !== undefined) {
	    		attrs[a] = args[a]
	    		delete args[a]
